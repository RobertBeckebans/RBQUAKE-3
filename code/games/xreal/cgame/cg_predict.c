...
====================
*/
static void CG_ClipMoveToEntities( const vec3_t start, const vec3_t mins, const vec3_t maxs, const vec3_t end, int skipNumber, int mask, trace_t* tr, traceType_t collisionType )
{
	int			   i, x, zd, zu;
	trace_t		   trace;
	entityState_t* ent;
...
================
*/
void CG_CapTrace( trace_t* result, const vec3_t start, const vec3_t mins, const vec3_t maxs, const vec3_t end, int skipNumber, int mask )
{
	trace_t t;
...
static void CG_TouchTriggerPrediction( void )
{
	...
	for( i = 0; i < cg_numTriggerEntities; i++ )
	{
		...
		trap_CM_TransformedCapsuleTrace( &trace, cg.predictedPlayerState.origin, cg.predictedPlayerState.origin, cg_pmove.mins, cg_pmove.maxs, cmodel, -1, origin, angles );
		...
		if( cg.predictedPlayerState.commandTime == oldPlayerState.commandTime )
		{
			...
			else
			{
				...
				CG_AdjustPositionForMover(
					cg.predictedPlayerState.origin, cg.predictedPlayerState.groundEntityNum, cg.physicsTime, cg.oldTime, adjusted, cg.predictedPlayerState.viewangles, new_angles );
