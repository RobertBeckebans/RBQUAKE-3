...
void BotCTFSeekGoals( bot_state_t* bs )
{
	...
	else if( flagstatus == 2 )
	{
		...
		if( bs->owndecision_time < FloatTime() )
		{
			...
			if( bs->ltgtype != LTG_GETFLAG && bs->ltgtype != LTG_RETURNFLAG && bs->ltgtype != LTG_TEAMHELP && bs->ltgtype != LTG_TEAMACCOMPANY && bs->ltgtype != LTG_CAMPORDER &&
	...
	if( bs->ltgtype == LTG_TEAMHELP || bs->ltgtype == LTG_TEAMACCOMPANY || bs->ltgtype == LTG_DEFENDKEYAREA || bs->ltgtype == LTG_GETFLAG || bs->ltgtype == LTG_RUSHBASE ||
		bs->ltgtype == LTG_RETURNFLAG || bs->ltgtype == LTG_CAMPORDER || bs->ltgtype == LTG_PATROL || bs->ltgtype == LTG_GETITEM || bs->ltgtype == LTG_MAKELOVE_UNDER ||
...
void Bot1FCTFSeekGoals( bot_state_t* bs )
{
	...
	if( bs->neutralflagstatus == 1 )
	{
		if( bs->owndecision_time < FloatTime() )
		{
			...
			// if already a CTF or team goal
			if( bs->ltgtype == LTG_TEAMHELP || bs->ltgtype == LTG_TEAMACCOMPANY || bs->ltgtype == LTG_DEFENDKEYAREA || bs->ltgtype == LTG_GETFLAG || bs->ltgtype == LTG_RUSHBASE ||
				bs->ltgtype == LTG_CAMPORDER || bs->ltgtype == LTG_PATROL || bs->ltgtype == LTG_ATTACKENEMYBASE || bs->ltgtype == LTG_GETITEM || bs->ltgtype == LTG_MAKELOVE_UNDER ||
				bs->ltgtype == LTG_MAKELOVE_ONTOP )
			{
				return;
			}
	...
	// if already a CTF or team goal
	if( bs->ltgtype == LTG_TEAMHELP || bs->ltgtype == LTG_TEAMACCOMPANY || bs->ltgtype == LTG_DEFENDKEYAREA || bs->ltgtype == LTG_GETFLAG || bs->ltgtype == LTG_RUSHBASE ||
		bs->ltgtype == LTG_RETURNFLAG || bs->ltgtype == LTG_CAMPORDER || bs->ltgtype == LTG_PATROL || bs->ltgtype == LTG_ATTACKENEMYBASE || bs->ltgtype == LTG_GETITEM ||
...
void BotHarvesterSeekGoals( bot_state_t* bs )
{
	...
	if( bs->ltgtype == LTG_TEAMHELP || bs->ltgtype == LTG_TEAMACCOMPANY || bs->ltgtype == LTG_DEFENDKEYAREA || bs->ltgtype == LTG_GETFLAG || bs->ltgtype == LTG_CAMPORDER ||
		bs->ltgtype == LTG_PATROL || bs->ltgtype == LTG_ATTACKENEMYBASE || bs->ltgtype == LTG_HARVEST || bs->ltgtype == LTG_GETITEM || bs->ltgtype == LTG_MAKELOVE_UNDER ||
		bs->ltgtype == LTG_MAKELOVE_ONTOP )
	{
		return;
	}
...
int BotWantsToCamp( bot_state_t* bs )
{
	...
	if( bs->ltgtype == LTG_TEAMHELP || bs->ltgtype == LTG_TEAMACCOMPANY || bs->ltgtype == LTG_DEFENDKEYAREA || bs->ltgtype == LTG_GETFLAG || bs->ltgtype == LTG_RUSHBASE || bs->ltgtype == LTG_CAMP ||
	...
	if( ( bs->inventory[INVENTORY_ROCKETLAUNCHER] <= 0 || bs->inventory[INVENTORY_ROCKETS] < 10 ) && ( bs->inventory[INVENTORY_RAILGUN] <= 0 || bs->inventory[INVENTORY_SLUGS] < 10 ) &&
...
int BotGetActivateGoal( bot_state_t* bs, int entitynum, bot_activategoal_t* activategoal )
{
	...
	for( i = 0; i >= 0 && i < 10; )
	{
		...
		if( !strcmp( classname, "func_button" ) )
		{
			...
			if( bs->activatestack && bs->activatestack->inuse && bs->activatestack->goal.entitynum == activategoal->goal.entitynum && bs->activatestack->time > FloatTime() &&
...
void BotCheckForProxMines( bot_state_t* bs, entityState_t* state )
{
	...
	if( !( bs->inventory[INVENTORY_PLASMAGUN] > 0 && bs->inventory[INVENTORY_CELLS] > 0 ) && !( bs->inventory[INVENTORY_ROCKETLAUNCHER] > 0 && bs->inventory[INVENTORY_ROCKETS] > 0 ) &&
...
void BotSetupAlternativeRouteGoals()
{
	...
	if( gametype == GT_CTF )
	{
		...
		{
			//
			red_numaltroutegoals  = trap_AAS_AlternativeRouteGoals( ctf_neutralflag.origin,
			...
			blue_numaltroutegoals = trap_AAS_AlternativeRouteGoals( ctf_neutralflag.origin,
				ctf_neutralflag.areanum,
				ctf_blueflag.origin,
	...
	else if( gametype == GT_OBELISK )
	{
		...
		//
		red_numaltroutegoals  = trap_AAS_AlternativeRouteGoals( neutralobelisk.origin,
			 ...
			 MAX_ALTROUTEGOALS,
			 ALTROUTEGOAL_CLUSTERPORTALS | ALTROUTEGOAL_VIEWPORTALS );
		blue_numaltroutegoals = trap_AAS_AlternativeRouteGoals( neutralobelisk.origin,
			neutralobelisk.areanum,
			blueobelisk.origin,
			blueobelisk.areanum,
			TFL_DEFAULT,
