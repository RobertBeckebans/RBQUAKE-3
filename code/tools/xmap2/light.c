...
int LightContributionToPoint( trace_t* trace )
{
	...
	if( trace->origin[0] > light->maxs[0] || trace->origin[0] < light->mins[0] || trace->origin[1] > light->maxs[1] || trace->origin[1] < light->mins[1] || trace->origin[2] > light->maxs[2] ||
		trace->origin[2] < light->mins[2] )
	{
		gridBoundsCulled++;
		return qfalse;
	}
...
void SetupGrid()
{
	...
	/* print it */
	Sys_Printf(
		"BSP bounds = { %1.0f, %1.0f, %1.0f }\n", ( bspModels[0].mins[0] + bspModels[0].maxs[0] ), ( bspModels[0].mins[1] + bspModels[0].maxs[1] ), ( bspModels[0].mins[2] + bspModels[0].maxs[2] ) );
