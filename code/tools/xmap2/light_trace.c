...
static int		 AddTraceInfo( traceInfo_t* ti )
{
	...
	{
		if( traceInfos[num].si == ti->si && traceInfos[num].surfaceNum == ti->surfaceNum && traceInfos[num].castShadows == ti->castShadows && traceInfos[num].skipGrid == ti->skipGrid )
		{
			return num;
		}
	}
...
static void PopulateWithBSPModel( bspModel_t* model, matrix_t transform )
{
	...
	for( i = 0; i < model->numBSPSurfaces; i++ )
	{
		...
		if( ( bspShaders[ds->shaderNum].contentFlags & noDrawContentFlags ) || ( bspShaders[ds->shaderNum].surfaceFlags & noDrawSurfaceFlags ) )
		{
			continue;
		}
		...
		if( ( info->si->compileFlags & C_TRANSLUCENT ) && !( info->si->compileFlags & C_ALPHASHADOW ) && !( info->si->compileFlags & C_LIGHTFILTER ) )
		{
			continue;
		}
...
static void PopulateWithPicoModel( int castShadows, picoModel_t* model, matrix_t transform )
{
	...
	for( i = 0; i < numSurfaces; i++ )
	{
		...
		if( ( ti.si->compileFlags & C_TRANSLUCENT ) && !( ti.si->compileFlags & C_ALPHASHADOW ) && !( ti.si->compileFlags & C_LIGHTFILTER ) )
		{
			continue;
		}
