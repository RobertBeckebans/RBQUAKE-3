...
static md3Header_t* MD3_Load( const char* mod_name )
{
	...
	for( i = 0; i < md3->numSurfaces; i++ )
	{
		...
		{
			Error( "MD3_Load: %s has more than %i triangles on a surface (%i)", mod_name, SHADER_MAX_INDEXES / 3, surf->numTriangles );
		}
...
static void InsertASEModel( const char* modelName, const matrix_t transform )
{
	...
	for( i = 0; i < numSurfaces; i++ )
	{
		...
		for( j = 0; j < pset->numtriangles * 3; j++ )
		{
			...
			{
				if( vertexes[k].xyz[0] != vertex.xyz[0] || vertexes[k].xyz[1] != vertex.xyz[k] || vertexes[k].xyz[2] != vertex.xyz[2] )
				{
					continue;
				}
	...
	for( i = 0, surf = obj->surf; i < obj->nsurfs; i++, surf = surf->next )
	{
		...
		for( j = 0; j < layer->polygon.count; j++ )
		{
			...
			for( k = 0, v = pol->v; k < pol->nverts; k++, v++ )
			{
				...
				{
					if( vertexes[l].xyz[0] != vertex.xyz[0] || vertexes[l].xyz[1] != vertex.xyz[1] || vertexes[l].xyz[2] != vertex.xyz[2] )
					{
						continue;
					}
