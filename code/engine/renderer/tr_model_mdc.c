...
qboolean R_LoadMDC( model_t* mod, int lod, void* buffer, int bufferSize, const char* modName )
{
	...
	for( i = 0; i < mdcModel->numSurfaces; i++ )
	{
		...
		{
			ri.Error( ERR_DROP, "R_LoadMDC: %s has more than %i verts on a surface (%i)", modName, SHADER_MAX_VERTEXES, mdcSurf->numVerts );
		}
		if( mdcSurf->numTriangles > SHADER_MAX_TRIANGLES )
		{
			ri.Error( ERR_DROP, "R_LoadMDC: %s has more than %i triangles on a surface (%i)", modName, SHADER_MAX_TRIANGLES, mdcSurf->numTriangles );
		}
