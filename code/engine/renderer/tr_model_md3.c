...
qboolean R_LoadMD3( model_t* mod, int lod, void* buffer, int bufferSize, const char* modName )
{
	...
	for( i = 0; i < md3Model->numSurfaces; i++ )
	{
		...
		{
			ri.Error( ERR_DROP, "R_LoadMD3: %s has more than %i verts on a surface (%i)", modName, SHADER_MAX_VERTEXES, md3Surf->numVerts );
		}
		if( md3Surf->numTriangles * 3 > SHADER_MAX_INDEXES )
		{
			ri.Error( ERR_DROP, "R_LoadMD3: %s has more than %i triangles on a surface (%i)", modName, SHADER_MAX_INDEXES / 3, md3Surf->numTriangles );
		}
