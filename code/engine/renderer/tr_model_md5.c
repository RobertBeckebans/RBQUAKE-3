...
qboolean R_LoadMD5( model_t* mod, void* buffer, int bufferSize, const char* modName )
{
	...
	for( i = 0, surf = md5->surfaces; i < md5->numSurfaces; i++, surf++ )
	{
		...
		{
			ri.Error( ERR_DROP, "R_LoadMD5: '%s' has more than %i verts on a surface (%i)", modName, SHADER_MAX_VERTEXES, surf->numVerts );
		}
		...
		for( j = 0, v = surf->verts; j < surf->numVerts; j++, v++ )
		{
			...
			{
				ri.Error( ERR_DROP, "R_LoadMD5: vertex %i requires more than %i weights on surface (%i) in model '%s'", j, MAX_WEIGHTS, i, modName );
			}
		}
		...
		{
			ri.Error( ERR_DROP, "R_LoadMD5: '%s' has more than %i triangles on a surface (%i)", modName, SHADER_MAX_TRIANGLES, surf->numTriangles );
		}
