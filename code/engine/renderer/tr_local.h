...
#define DS_STANDARD_ENABLED()                                                                                                                                 \
	( ( r_deferredShading->integer == DS_STANDARD && glConfig2.maxColorAttachments >= 4 && glConfig2.drawBuffersAvailable && glConfig2.maxDrawBuffers >= 4 && \
		/*glConfig2.framebufferPackedDepthStencilAvailable &&*/ glConfig.driverType != GLDRV_MESA ) )

#define HDR_ENABLED() \
	( ( r_hdrRendering->integer && glConfig2.textureFloatAvailable && glConfig2.framebufferObjectAvailable && glConfig2.framebufferBlitAvailable && glConfig.driverType != GLDRV_MESA ) )
...
enum
{
	...
	enum
	{
		...
		GLS_SRCBLEND_BITS = GLS_SRCBLEND_ZERO | GLS_SRCBLEND_ONE | GLS_SRCBLEND_DST_COLOR | GLS_SRCBLEND_ONE_MINUS_DST_COLOR | GLS_SRCBLEND_SRC_ALPHA | GLS_SRCBLEND_ONE_MINUS_SRC_ALPHA |
							GLS_SRCBLEND_DST_ALPHA | GLS_SRCBLEND_ONE_MINUS_DST_ALPHA | GLS_SRCBLEND_ALPHA_SATURATE,
		...
		GLS_DSTBLEND_BITS = GLS_DSTBLEND_ZERO | GLS_DSTBLEND_ONE | GLS_DSTBLEND_SRC_COLOR | GLS_DSTBLEND_ONE_MINUS_SRC_COLOR | GLS_DSTBLEND_SRC_ALPHA | GLS_DSTBLEND_ONE_MINUS_SRC_ALPHA |
		...
		GLS_ATEST_BITS = GLS_ATEST_GT_0 | GLS_ATEST_LT_128 | GLS_ATEST_GE_128,
		//											| GLS_ATEST_GT_CUSTOM,
		...
		GLS_COLORMASK_BITS = GLS_REDMASK_FALSE | GLS_GREENMASK_FALSE | GLS_BLUEMASK_FALSE | GLS_ALPHAMASK_FALSE,
...
enum
{
	...
	ATTR_BITS = ATTR_POSITION | ATTR_TEXCOORD | ATTR_LIGHTCOORD | ATTR_TANGENT | ATTR_BINORMAL | ATTR_NORMAL | ATTR_COLOR
...
void		   GLimp_LogComment( const char* comment );
	...
	static ID_INLINE void GLSL_SetUniform_ColorTextureMatrix( shaderProgram_t* program, const matrix_t m )
	{
		...
		if( MatrixCompare( program->t_ColorTextureMatrix, m ) ) return;
	...
	{
	#if defined( USE_UNIFORM_FIREWALL )
		if( MatrixCompare( program->t_DiffuseTextureMatrix, m ) ) return;
	...
	{
	#if defined( USE_UNIFORM_FIREWALL )
		if( MatrixCompare( program->t_NormalTextureMatrix, m ) ) return;
	...
	{
	#if defined( USE_UNIFORM_FIREWALL )
		if( MatrixCompare( program->t_SpecularTextureMatrix, m ) ) return;
	...
	#if defined( USE_UNIFORM_FIREWALL )
		if( program->t_AlphaTest == value ) return;
	...
	{
	#if defined( USE_UNIFORM_FIREWALL )
		if( VectorCompare( program->t_ViewOrigin, v ) ) return;

		VectorCopy( v, program->t_ViewOrigin );
	#endif

	#if defined( LOG_GLSL_UNIFORMS )
		if( r_logFile->integer ) { GLimp_LogComment( va( "--- GLSL_SetUniform_ViewOrigin( program = %s, viewOrigin = ( %5.3f, %5.3f, %5.3f ) ) ---\n", program->name, v[0], v[1], v[2] ) ); }
	#endif
	...
	{
	#if defined( USE_UNIFORM_FIREWALL )
		if( Vector4Compare( program->t_Color, v ) ) return;

		Vector4Copy( v, program->t_Color );
	#endif

	#if defined( LOG_GLSL_UNIFORMS )
		if( r_logFile->integer ) { GLimp_LogComment( va( "--- GLSL_SetUniform_Color( program = %s, color = ( %5.3f, %5.3f, %5.3f, %5.3f ) ) ---\n", program->name, v[0], v[1], v[2], v[3] ) ); }
	#endif
	...
	{
	#if defined( USE_UNIFORM_FIREWALL )
		if( Vector4Compare( program->t_ColorModulate, v ) ) return;

		Vector4Copy( v, program->t_ColorModulate );
	#endif

	#if defined( LOG_GLSL_UNIFORMS )
		if( r_logFile->integer ) { GLimp_LogComment( va( "--- GLSL_SetUniform_ColorModulate( program = %s, color = ( %5.3f, %5.3f, %5.3f, %5.3f ) ) ---\n", program->name, v[0], v[1], v[2], v[3] ) ); }
	#endif
	...
	{
	#if defined( USE_UNIFORM_FIREWALL )
		if( VectorCompare( program->t_AmbientColor, v ) ) return;

		VectorCopy( v, program->t_AmbientColor );
	#endif

	#if defined( LOG_GLSL_UNIFORMS )
		if( r_logFile->integer ) { GLimp_LogComment( va( "--- GLSL_SetUniform_AmbientColor( program = %s, color = ( %5.3f, %5.3f, %5.3f ) ) ---\n", program->name, v[0], v[1], v[2] ) ); }
	#endif
	...
	{
	#if defined( USE_UNIFORM_FIREWALL )
		if( VectorCompare( program->t_LightDir, v ) ) return;

		VectorCopy( v, program->t_LightDir );
	#endif

	#if defined( LOG_GLSL_UNIFORMS )
		if( r_logFile->integer ) { GLimp_LogComment( va( "--- GLSL_SetUniform_LightDir( program = %s, direction = ( %5.3f, %5.3f, %5.3f ) ) ---\n", program->name, v[0], v[1], v[2] ) ); }
	#endif
	...
	{
	#if defined( USE_UNIFORM_FIREWALL )
		if( VectorCompare( program->t_LightOrigin, v ) ) return;

		VectorCopy( v, program->t_LightOrigin );
	#endif

	#if defined( LOG_GLSL_UNIFORMS )
		if( r_logFile->integer ) { GLimp_LogComment( va( "--- GLSL_SetUniform_LightOrigin( program = %s, origin = ( %5.3f, %5.3f, %5.3f ) ) ---\n", program->name, v[0], v[1], v[2] ) ); }
	#endif
	...
	{
	#if defined( USE_UNIFORM_FIREWALL )
		if( VectorCompare( program->t_LightColor, v ) ) return;

		VectorCopy( v, program->t_LightColor );
	#endif

	#if defined( LOG_GLSL_UNIFORMS )
		if( r_logFile->integer ) { GLimp_LogComment( va( "--- GLSL_SetUniform_LightColor( program = %s, color = ( %5.3f, %5.3f, %5.3f ) ) ---\n", program->name, v[0], v[1], v[2] ) ); }
	#endif
	...
	{
	#if defined( USE_UNIFORM_FIREWALL )
		if( program->t_LightRadius == value ) return;

		program->t_LightRadius = value;
	#endif

	#if defined( LOG_GLSL_UNIFORMS )
		if( r_logFile->integer ) { GLimp_LogComment( va( "--- GLSL_SetUniform_LightRadius( program = %s, value = %f ) ---\n", program->name, value ) ); }
	#endif
	...
	{
	#if defined( USE_UNIFORM_FIREWALL )
		if( program->t_LightParallel == value ) return;

		program->t_LightParallel = value;
	#endif

	#if defined( LOG_GLSL_UNIFORMS )
		if( r_logFile->integer ) { GLimp_LogComment( va( "--- GLSL_SetUniform_LightParallel( program = %s, value = %i ) ---\n", program->name, value ) ); }
	#endif
	...
	#if defined( USE_UNIFORM_FIREWALL )
		if( program->t_LightScale == value ) return;

		program->t_LightScale = value;
	#endif

	#if defined( LOG_GLSL_UNIFORMS )
		if( r_logFile->integer ) { GLimp_LogComment( va( "--- GLSL_SetUniform_LightScale( program = %s, value = %f ) ---\n", program->name, value ) ); }
	#endif
	...
	{
	#if defined( USE_UNIFORM_FIREWALL )
		if( program->t_LightWrapAround == value ) return;

		program->t_LightWrapAround = value;
	#endif

	#if defined( LOG_GLSL_UNIFORMS )
		if( r_logFile->integer ) { GLimp_LogComment( va( "--- GLSL_SetUniform_LightWrapAround( program = %s, value = %f ) ---\n", program->name, value ) ); }
	#endif
	...
	{
	#if defined( USE_UNIFORM_FIREWALL )
		if( MatrixCompare( program->t_LightAttenuationMatrix, m ) ) return;
	...
	*/
	...
	{
	#if defined( USE_UNIFORM_FIREWALL )
		if( program->t_ShadowCompare == value ) return;

		program->t_ShadowCompare = value;
	#endif

	#if defined( LOG_GLSL_UNIFORMS )
		if( r_logFile->integer ) { GLimp_LogComment( va( "--- GLSL_SetUniform_ShadowCompare( program = %s, value = %i ) ---\n", program->name, value ) ); }
	#endif
	...
	{
	#if defined( USE_UNIFORM_FIREWALL )
		if( program->t_ShadowTexelSize == value ) return;

		program->t_ShadowTexelSize = value;
	#endif

	#if defined( LOG_GLSL_UNIFORMS )
		if( r_logFile->integer ) { GLimp_LogComment( va( "--- GLSL_SetUniform_ShadowTexelSize( program = %s, value = %f ) ---\n", program->name, value ) ); }
	#endif
	...
	{
	#if defined( USE_UNIFORM_FIREWALL )
		if( program->t_ShadowBlur == value ) return;

		program->t_ShadowBlur = value;
	#endif

	#if defined( LOG_GLSL_UNIFORMS )
		if( r_logFile->integer ) { GLimp_LogComment( va( "--- GLSL_SetUniform_ShadowBlur( program = %s, value = %f ) ---\n", program->name, value ) ); }
	#endif
	...
	{
	#if defined( USE_UNIFORM_FIREWALL )
		if( Vector4Compare( program->t_ShadowParallelSplitDistances, v ) ) return;
	...
	{
	#if defined( USE_UNIFORM_FIREWALL )
		if( program->t_RefractionIndex == value ) return;

		program->t_RefractionIndex = value;
	#endif

	#if defined( LOG_GLSL_UNIFORMS )
		if( r_logFile->integer ) { GLimp_LogComment( va( "--- GLSL_SetUniform_RefractionIndex( program = %s, value = %f ) ---\n", program->name, value ) ); }
	#endif
	...
	{
	#if defined( USE_UNIFORM_FIREWALL )
		if( program->t_ParallaxMapping == value ) return;

		program->t_ParallaxMapping = value;
	#endif

	#if defined( LOG_GLSL_UNIFORMS )
		if( r_logFile->integer ) { GLimp_LogComment( va( "--- GLSL_SetUniform_ParallaxMapping( program = %s, value = %i ) ---\n", program->name, value ) ); }
	#endif
	...
	{
	#if defined( USE_UNIFORM_FIREWALL )
		if( program->t_DepthScale == value ) return;

		program->t_DepthScale = value;
	#endif

	#if defined( LOG_GLSL_UNIFORMS )
		if( r_logFile->integer ) { GLimp_LogComment( va( "--- GLSL_SetUniform_DepthScale( program = %s, value = %f ) ---\n", program->name, value ) ); }
	#endif
	...
	{
	#if defined( USE_UNIFORM_FIREWALL )
		if( program->t_EnvironmentInterpolation == value ) return;

		program->t_EnvironmentInterpolation = value;
	#endif

	#if defined( LOG_GLSL_UNIFORMS )
		if( r_logFile->integer ) { GLimp_LogComment( va( "--- GLSL_SetUniform_EnvironmentInterpolation( program = %s, value = %f ) ---\n", program->name, value ) ); }
	#endif
	...
	{
	#if defined( USE_UNIFORM_FIREWALL )
		if( program->t_PortalClipping == value ) return;

		program->t_PortalClipping = value;
	#endif

	#if defined( LOG_GLSL_UNIFORMS )
		if( r_logFile->integer ) { GLimp_LogComment( va( "--- GLSL_SetUniform_PortalClipping( program = %s, value = %i ) ---\n", program->name, value ) ); }
	#endif
	...
	{
	#if defined( USE_UNIFORM_FIREWALL )
		if( Vector4Compare( program->t_PortalPlane, v ) ) return;

		VectorCopy( v, program->t_PortalPlane );
	#endif

	#if defined( LOG_GLSL_UNIFORMS )
		if( r_logFile->integer ) { GLimp_LogComment( va( "--- GLSL_SetUniform_PortalPlane( program = %s, plane = ( %5.3f, %5.3f, %5.3f, %5.3f ) ) ---\n", program->name, v[0], v[1], v[2], v[3] ) ); }
	#endif
	...
	{
	#if defined( USE_UNIFORM_FIREWALL )
		if( program->t_PortalRange == value ) return;

		program->t_PortalRange = value;
	#endif

	#if defined( LOG_GLSL_UNIFORMS )
		if( r_logFile->integer ) { GLimp_LogComment( va( "--- GLSL_SetUniform_PortalRange( program = %s, value = %f ) ---\n", program->name, value ) ); }
	#endif
	...
	{
	#if defined( USE_UNIFORM_FIREWALL )
		if( MatrixCompare( program->t_ModelMatrix, m ) ) return;
	...
	{
	#if defined( USE_UNIFORM_FIREWALL )
		if( MatrixCompare( program->t_ViewMatrix, m ) ) return;
	...
	{
	#if defined( USE_UNIFORM_FIREWALL )
		if( MatrixCompare( program->t_ModelViewMatrix, m ) ) return;
	...
	{
	#if defined( USE_UNIFORM_FIREWALL )
		if( MatrixCompare( program->t_ModelViewMatrixTranspose, m ) ) return;
	...
	{
	#if defined( USE_UNIFORM_FIREWALL )
		if( MatrixCompare( program->t_ProjectionMatrix, m ) ) return;
	...
	{
	#if defined( USE_UNIFORM_FIREWALL )
		if( MatrixCompare( program->t_ProjectionMatrixTranspose, m ) ) return;
	...
	{
	#if defined( USE_UNIFORM_FIREWALL )
		if( MatrixCompare( program->t_ModelViewProjectionMatrix, m ) ) return;
	...
	{
	#if defined( USE_UNIFORM_FIREWALL )
		if( MatrixCompare( program->t_UnprojectMatrix, m ) ) return;
	...
	{
	#if defined( USE_UNIFORM_FIREWALL )
		if( program->t_VertexSkinning == value ) return;

		program->t_VertexSkinning = value;
	#endif

	#if defined( LOG_GLSL_UNIFORMS )
		if( r_logFile->integer ) { GLimp_LogComment( va( "--- GLSL_SetUniform_VertexSkinning( program = %s, value = %i ) ---\n", program->name, value ) ); }
	#endif
	...
	{
	#if defined( USE_UNIFORM_FIREWALL )
		if( program->t_VertexInterpolation == value ) return;

		program->t_VertexInterpolation = value;
	#endif

	#if defined( LOG_GLSL_UNIFORMS )
		if( r_logFile->integer ) { GLimp_LogComment( va( "--- GLSL_SetUniform_VertexInterpolation( program = %s, value = %f ) ---\n", program->name, value ) ); }
	#endif
	...
	{
	#if defined( USE_UNIFORM_FIREWALL )
		if( program->t_Time == value ) return;

		program->t_Time = value;
	#endif

	#if defined( LOG_GLSL_UNIFORMS )
		if( r_logFile->integer ) { GLimp_LogComment( va( "--- GLSL_SetUniform_Time( program = %s, value = %f ) ---\n", program->name, value ) ); }
	#endif
...
void			R_CalcTangentsForTriangle( vec3_t tangent, vec3_t binormal, const vec3_t v0, const vec3_t v1, const vec3_t v2, const vec2_t t0, const vec2_t t1, const vec2_t t2 );

void			R_CalcTangentsForTriangle2( vec3_t tangent, vec3_t binormal, const vec3_t v0, const vec3_t v1, const vec3_t v2, const vec2_t t0, const vec2_t t1, const vec2_t t2 );

void			R_CalcTangentSpace( vec3_t tangent, vec3_t binormal, vec3_t normal, const vec3_t v0, const vec3_t v1, const vec3_t v2, const vec2_t t0, const vec2_t t1, const vec2_t t2 );

void			R_CalcTangentSpaceFast( vec3_t tangent, vec3_t binormal, vec3_t normal, const vec3_t v0, const vec3_t v1, const vec3_t v2, const vec2_t t0, const vec2_t t1, const vec2_t t2 );

void			R_CalcTBN( vec3_t tangent, vec3_t binormal, vec3_t normal, const vec3_t v0, const vec3_t v1, const vec3_t v2, const vec2_t t0, const vec2_t t1, const vec2_t t2 );
...
int		 R_MDM_GetBoneTag( orientation_t* outTag, mdmModel_t* mdm, int startTagIndex, const refEntity_t* refent, const char* tagName );
