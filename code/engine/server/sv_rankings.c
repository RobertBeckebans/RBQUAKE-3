...
static const char       s_ascii_encoding[]  = "0123456789abcdef"
									   "ghijklmnopqrstuv"
									   "wxyzABCDEFGHIJKL"
									   "MNOPQRSTUVWXYZ[]";
...
void SV_RankBegin( char* gamekey )
{
	...
	{
		status = GRankNewGameAsync( s_server_context,
			SV_RankNewGameCBF,
			NULL,
			GR_OPT_LEAGUENAME,
...
void SV_RankUserCreate( int index, char* username, char* password, char* email )
{
	...
	status = GRankUserCreateAsync( s_ranked_players[ index ].context,
		username,
		password,
		email,
		SV_RankUserCBF,
