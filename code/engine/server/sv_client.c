...
void		SV_GetChallenge( netadr_t from )
{
	...
	if( challenge->adr.type == NA_IP && !com_standalone->integer && !Sys_IsLANAddress( from ) )
	{
		...
		else
		{
			...
			NET_OutOfBandPrint(
				NS_SERVER, svs.authorizeAddress, "getIpAuthorize %i %i.%i.%i.%i %s 0 %s", challenge->challenge, from.ip[0], from.ip[1], from.ip[2], from.ip[3], game, sv_strictAuth->string );
...
int SV_WriteDownloadToClient( client_t* cl, msg_t* msg )
{
	...
	if( !cl->download )
	{
		...
		if( !( sv_allowDownload->integer & DLF_ENABLE ) || ( sv_allowDownload->integer & DLF_NO_UDP ) || idPack || unreferenced ||
