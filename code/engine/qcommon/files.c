...
static const unsigned int pak_checksums[] = { 1566731103u, 298122907u, 412165236u, 2991495316u, 1197932710u, 4087071573u, 3709064859u, 908855077u, 977125798u };

static const unsigned int missionpak_checksums[] = { 2430342401u, 511014160u, 2662638993u, 1438664554u };
#endif
...
static void FS_CheckFilenameIsMutable( const char* filename, const char* function )
{
	...
	if( Sys_DllExtension( filename ) || Com_CompareExtension( filename, ".qvm" ) || Com_CompareExtension( filename, ".pk3" ) || Com_CompareExtension( filename, DLL_EXT ) )
	{
		Com_Error( ERR_FATAL,
...
#endif
							!FS_IsExt( filename, ".txt", len ) && !FS_IsExt( filename, ".cfg", len ) && !FS_IsExt( filename, ".config", len ) && !FS_IsExt( filename, ".bot", len ) &&
							!FS_IsExt( filename, ".arena", len ) && !FS_IsExt( filename, ".menu", len ) && !strstr( filename, "levelshots" ) )
						{
...
static void FS_CheckPak0()
{
	...
	for( path = fs_searchpaths; path; path = path->next )
	{
		...
		else if( !Q_stricmpn( curpack->pakGamename, BASEGAME, MAX_OSPATH ) && strlen( pakBasename ) == 4 && !Q_stricmpn( pakBasename, "pak", 3 ) && pakBasename[3] >= '0' &&
		...
		else if( !Q_stricmpn( curpack->pakGamename, BASETA, MAX_OSPATH ) && strlen( pakBasename ) == 4 && !Q_stricmpn( pakBasename, "pak", 3 ) && pakBasename[3] >= '0' &&
...
qboolean FS_ConditionalRestart( int checksumFeed, qboolean disconnect )
{
	...
	{
		if( FS_FilenameCompare( lastValidGame, fs_gamedirvar->string ) && ( *lastValidGame || FS_FilenameCompare( fs_gamedirvar->string, com_basegame->string ) ) &&
